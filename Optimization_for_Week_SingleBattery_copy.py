...
G=0.09 ## un Coeff G qui depend du batiment
...
Lundi=10*np.ones((48,1))
Mardi=20*np.ones((48,1))
Mercredi=20*np.ones((48,1))
Jeudi=20*np.ones((48,1))
Vendredi=20*np.ones((48,1))
Samedi=20*np.ones((48,1))
ConsigneB=np.concatenate([Lundi,Mardi,Mercredi,Jeudi,Vendredi,Samedi],axis=0)

LundiH=24*np.ones((48,1))
MardiH=24*np.ones((48,1))
MercrediH=24*np.ones((48,1))
JeudiH=24*np.ones((48,1))
VendrediH=24*np.ones((48,1))
SamediH=24*np.ones((48,1))
ConsigneH=np.concatenate([LundiH,MardiH,MercrediH,JeudiH,VendrediH,SamediH],axis=0)

T=np.concatenate([Treel,Treel],axis=0)
#ConsigneB=20*np.ones((288,1))
#ConsigneH=24*np.ones((288,1))
Tint0=22;
...
for i in range(0,len(P),Length):# we iterate every 24 hours for th whole week
    ...
    # Tint0Mat=np.zeros((LengthOptim,LengthOptim))
    # Tint0Mat[0][0]=Tint0
    gammaMat=-gamma*np.eye(LengthOptim)
    gammaMat[0][0]=0
    beq3Mat=beta*np.ones(LengthOptim)
    beq3Mat[0]=Tint0
    
    f =np.concatenate([BPrice[:,0],-SPrice[:,0],np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim)]);#coeffconfort*np.ones((LengthOptim))
    ...
    Aeq3=np.concatenate([np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),gammaMat,-alpha*np.eye((LengthOptim),k=-1)+np.eye(LengthOptim),],axis=1)
    ...
    A5=np.concatenate([np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,-np.tril(np.ones((LengthOptim,LengthOptim))) ,np.tril(np.ones((LengthOptim,LengthOptim))) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),],axis=1)
    A6=np.concatenate([np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.tril(np.ones((LengthOptim,LengthOptim))) ,-np.tril(np.ones((LengthOptim,LengthOptim))) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),],axis=1)
    ...
    A10=np.concatenate([np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim)),],axis=1)
    A =np.concatenate([A1,A2,A3,A4,A5,A6,A7,A8,A9,A10],axis=0)
        
    Beq1=Demand- Production
    Beq2=Production-Demand
    Beq3=beq3Mat
    #Beq4=22*np.ones(LengthOptim) 
    
    # Ti=np.zeros((48,48))#np.eye((LengthOptim),k=-1)
    # Ti[0][0]=Tint0
    # for j in range(1,48):
    #     Ti[j][j]=Ti[j-1][j-1]*2
    ...
    b = np.concatenate([np.zeros((LengthOptim,1)), Pmax*np.ones((LengthOptim,1)), np.zeros((LengthOptim,1)), Pmax*np.ones((LengthOptim,1)), (SoCmax-SoCInit)*np.ones((LengthOptim,1)), (SoCInit - SoCmin)*np.ones((LengthOptim,1)), np.zeros((LengthOptim,1)), Pbatmax*np.ones((LengthOptim,1)),Pmax*np.ones((LengthOptim,1)),np.zeros((LengthOptim,1))])
    lb =np.concatenate([np.zeros((LengthOptim,1)),np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)),  np.zeros((LengthOptim,1)),  np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)),np.zeros((LengthOptim,1)),ConsigneBasse])
    ub = np.concatenate([Pmax*np.ones((LengthOptim,1)),Pmax*np.ones((LengthOptim,1)), Pbatmax*np.ones((LengthOptim,1)),  Pbatmax*np.ones((LengthOptim,1)), Pmax*np.ones((LengthOptim,1)),  Pmax*np.ones((LengthOptim,1)),  np.ones((LengthOptim,1)),  np.ones((LengthOptim,1)),Pmax*np.ones((LengthOptim,1)),ConsigneHaute])
    b=b[:,0]
...
#     #             options.Display = 'off';
    ...
    model=gp.Model()
    x=model.addMVar(480,ub=ub,lb=lb)
    x.obj=f
