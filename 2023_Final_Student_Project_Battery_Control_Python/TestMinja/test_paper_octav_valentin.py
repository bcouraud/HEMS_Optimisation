...
for i in range(0,len(P),Length):# we iterate every 24 hours for the whole week
    ...
    f =np.r_[BPrice[:,0],  #imports from grid
             ...
             lambdac*np.ones(LengthOptim), # Discharge power from battery
             lambdac*np.ones(LengthOptim), #charging power from battery
    ...
    Index_binary =np.r_[np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),np.zeros(LengthOptim),0,np.ones(LengthOptim),np.ones(LengthOptim)]

    Aeq1=np.c_[np.eye(LengthOptim), np.zeros((LengthOptim,LengthOptim)), EfficiencyD*np.eye(LengthOptim), -np.eye(LengthOptim)/EfficiencyC, -np.eye(LengthOptim),np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,1)),np.zeros((LengthOptim,LengthOptim)), np.zeros((LengthOptim,LengthOptim))]
    Aeq2=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.eye(LengthOptim), -EfficiencyD*np.eye(LengthOptim) ,np.eye(LengthOptim)/EfficiencyC ,np.zeros((LengthOptim,LengthOptim)) ,-np.eye(LengthOptim),np.zeros((LengthOptim,1)),np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,LengthOptim))]
    Aeq3=np.c_[np.zeros((1,LengthOptim)) ,np.zeros((1,LengthOptim)), np.ones((1,LengthOptim)) ,-np.ones((1,LengthOptim)) ,np.zeros((1,LengthOptim)) ,np.zeros((1,LengthOptim)),1,np.zeros((1,LengthOptim)),np.zeros((1,LengthOptim))] #SOC final = SocInit -Pdischarge + Pcharge 
    Aeq =np.concatenate([Aeq1,Aeq2,Aeq3],axis=0)
    Beq =np.concatenate([ Demand - Production , Production - Demand , SoCInit*np.ones(1)],axis=0)
        
        
    A1=np.c_[np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,1)) ,-Pmax*np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim))]
    A2=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,1)) ,Pmax*np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim))]
    A3=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.eye(LengthOptim),np.zeros((LengthOptim,1)),-Pmax*np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim))]
    A4=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,1)),Pmax*np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim))]
    A5=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,-np.tril(np.ones((LengthOptim,LengthOptim))) ,np.tril(np.ones((LengthOptim,LengthOptim))) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,1)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim))]
    A6=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.tril(np.ones((LengthOptim,LengthOptim))) ,-np.tril(np.ones((LengthOptim,LengthOptim))) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,1)),np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim))]
    A7=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)),np.zeros((LengthOptim,1)) ,np.zeros((LengthOptim,LengthOptim)) ,-Pbatmax*np.eye(LengthOptim)]
    A8=np.c_[np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.eye(LengthOptim) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,LengthOptim)) ,np.zeros((LengthOptim,1)),np.zeros((LengthOptim,LengthOptim)) ,Pbatmax*np.eye(LengthOptim)]
    ...
    lb =np.r_[np.zeros((LengthOptim,1)),np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1)),  np.zeros((LengthOptim,1)), SoCmin*np.ones((1,1)), np.zeros((LengthOptim,1)), np.zeros((LengthOptim,1))]
    ub = np.r_[Pmax*np.ones((LengthOptim,1)),700*np.ones((LengthOptim,1)), Pbatmax*np.ones((LengthOptim,1)),  Pbatmax*np.ones((LengthOptim,1)), Pmax*np.ones((LengthOptim,1)),  Pmax*np.ones((LengthOptim,1)), SoCmax*np.ones((1,1)), np.ones((LengthOptim,1)),  np.ones((LengthOptim,1))]
    ...
    SoCfinal = solution[LengthOptim*6:LengthOptim*6+1]
    Alpha = solution[LengthOptim*6+1:LengthOptim*7+1]
    Beta = solution[LengthOptim*7+1:LengthOptim*8+2]
    SoCfinal2 = SoC[i+LengthOptim-1]
